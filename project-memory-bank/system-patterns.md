...
- `vite.config.js`: Vite + Tailwind + `vite-plugin-pwa` + Vitest configuration (manifest fields, Workbox precache glob, `registerType: 'prompt'`, `esbuild.jsx: 'automatic'` for the test transform pipeline, `test.setupFiles`).
- `AI-Lexicon.test.jsx` / `src/hooks/useLexiconData.test.js` (Phase 7): critical-path component/hook tests using `@testing-library/react`, alongside the existing pure-lib Vitest suites.
- `src/test/setup.js` (Phase 7): registers `@testing-library/jest-dom` matchers globally for Vitest.
- `docs/screenshots/` (Phase 7): static PNGs embedded in README's Demo section.
...
- `useDialogA11y` (Phase 7): reusable focus-management hook shared by all four dialogs/drawers. Given `(isOpen, onClose, initialFocusRef?)`, it: captures the pre-open "trigger" element synchronously during render (not in an effect — see System Patterns note below on why), moves focus to `initialFocusRef.current` (if provided) or the first focusable element inside the dialog once open, traps `Tab`/`Shift+Tab` cycling within the dialog, closes on `Escape`, and restores focus to the trigger element on close/unmount. Returns a `containerRef` the dialog attaches to its outermost element.
...
- **Accessibility (Phase 7)**: every icon-only button has an `aria-label` (derived from the item's title, e.g. `Archive ${section.title}`) in addition to its hover `title`. Expand/collapse toggles (`SectionPanel`, `CardPanel`) set `aria-expanded={Boolean(expanded)}` — coerced explicitly, since `expandedSections[id]`/`expandedItems[id]` are `undefined` (not `false`) before first toggle, and React omits `undefined` attributes entirely, which would otherwise leave the ARIA state absent rather than `"false"`. Filter toggles (`FilterBar`'s favorites button and tag chips) set `aria-pressed`. `InlineBanner` renders `role="alert"` (errors) or `role="status"` (info/success) with a matching `aria-live`, so all five banner types (save error, import error, offline notice, update-available, offline-ready) are announced automatically. All four dialogs/drawers use `useDialogA11y` (see Hooks Layer) for focus trapping, initial focus, Escape-to-close, and focus restoration — `EditModal` and `TemplateFillModal` pass an explicit `initialFocusRef` (title input / first template-variable input) instead of relying on native `autoFocus`, because `autoFocus`'s browser-driven timing raced with `useDialogA11y`'s own effect-based focus logic (see Recent Decisions in `active-context.md`). A "Skip to content" link (Tailwind `sr-only`/`focus:not-sr-only`) is the first element in `AI-Lexicon.jsx`'s render tree, targeting a new `id="main-content"` on `<main>`.
